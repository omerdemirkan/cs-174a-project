...
export class GameScene extends Scene {
  constructor() {
    ...
    this.materials = {
      ...
      pac1: new Material(new Cartoon_Phong(), {
        ambient: 0.75,
        specularity: 0,
      ...
      center_rail: new Material(new defs.Fake_Bump_Map(5), {
        ...
        diffusivity: 1,
        specularity: 1,
        texture: new Texture("assets/cubeshade.png", "LINEAR_MIPMAP_LINEAR"),
      }),
      ...
      wall_floor: new Material(new defs.Fake_Bump_Map(5), {
        ...
        diffusivity: 0.5,
        specularity: 0.3,
        texture: new Texture("assets/scaled_walls.png", "LINEAR_MIPMAP_LINEAR"),
      }),
      ...
      trail: new Material(new defs.Fake_Bump_Map(5), {
        ...
        diffusivity: 1,
        specularity: 1,
        texture: new Texture("assets/glow_trail.png", "LINEAR_MIPMAP_LINEAR"),
        color: color(1, 1, 1, 1),
      }),
      ...
      trail_floors: new Material(new Textured_Phong(5), {
        ...
        diffusivity: 0,
        specularity: 0,
        texture: new Texture("assets/floors.png", "LINEAR_MIPMAP_LINEAR"),
      }),
          ...
          this.shapes.box.draw(
            ...
            Mat4.translation((x + x_mod) * 2, (y + y_mod) * 2, z).times(
              Mat4.scale(
                1 / 3,
                1 / 3,
                this.colorCube[index][i][j] == colors.white
                  ? 1
                  : 1 + Math.abs(Math.sin(t + (x + x_mod) + (y + y_mod)))
              )
            ),
            mat_shad
          );
  ...
  display(context, program_state) {
    ...
    if ((t * PACMAN_CHOMPS_PER_SECOND) % 1 < 0.5) {
      this.shapes.pacman.draw(
        ...
        this.materials.pac0.override({
          ambient:
            Date.now() - playerState.lastPowerUpEatenMillisecond <
            this.game._PACMAN_ON_SMOKE_DURATION_MS - 1500
              ? 1
              : 0.75,
        })
      );
      ...
      this.shapes.pacman.draw(
        ...
        this.materials.pac1.override({
          ambient: playerState.isOnSmoke ? 1 : 0.75,
        })
      );
    }
    ...
    this.shapes.box.draw(
      ...
      this.materials.trail_floors.override({
        color: color(Math.sin(t + Math.PI), Math.cos(t), Math.sin(t), 0.5),
      })
    );
    this.shapes.box.draw(
      context,
...
class Cartoon_Phong extends defs.Phong_Shader {
